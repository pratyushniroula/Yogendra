const canvas = document.getElementById("gameCanvas"),
  ...
  paddle = { radius: 25, y: canvas.height - 50 },
  aiPaddle = { radius: 25, y: 50, speed: 3.6 },
  puck = { radius: 15, speed: 9 },
  goal = { width: 150, height: 10 };
...
let playerScore = 0,
  ...
  aiWaitTime = 0,
  aiState = "normal",
  goalEffect = { active: false, startTime: 0, scorer: "" },
  particles = [];
let lastTime = 0;
const targetFPS = 60;
...
function launchPuck() {
  ...
  puck.dx = Math.cos(a) * puck.speed;
  puck.dy = Math.sin(a) * puck.speed;
}
...
function movePaddle(x, y) {
  ...
  paddle.x = Math.min(
    Math.max(x - r.left, paddle.radius),
    canvas.width - paddle.radius
  );
  paddle.y = Math.min(
    Math.max(y - r.top, canvas.height / 2 + paddle.radius),
    canvas.height - paddle.radius
  );
}
...
function isPuckStuck() {
  return (
    puck.y < canvas.height / 4 &&
    (puck.x < puck.radius * 2 || puck.x > canvas.width - puck.radius * 2)
  );
}

function moveAIPaddle() {
  const c = aiPaddle.x,
    d = Math.abs(c - canvas.width / 2);
  ...
  if (aiState === "retreating") {
    ...
    if (distance > aiPaddle.speed) {
      aiPaddle.x +=
        (dx / distance) * aiPaddle.speed * simulationSpeedMultiplier;
      aiPaddle.y +=
        (dy / distance) * aiPaddle.speed * simulationSpeedMultiplier;
    } else {
      aiPaddle.x = targetX;
      aiPaddle.y = targetY;
      aiState = "waiting";
    ...
    if (puck.y < canvas.height / 2) {
      if (Math.abs(puck.x - c) > 5)
        puck.x > c
          ? (aiPaddle.x += aiPaddle.speed * simulationSpeedMultiplier)
          : (aiPaddle.x -= aiPaddle.speed * simulationSpeedMultiplier);
      puck.y > aiPaddle.y && Math.abs(puck.x - c) < 50
        ? (aiPaddle.y += aiPaddle.speed * simulationSpeedMultiplier)
        : (aiPaddle.y += (50 - aiPaddle.y) * 0.05 * simulationSpeedMultiplier);
    } else {
      d > 70
        ? (aiPaddle.x +=
            (canvas.width / 2 - c) * 0.03 * simulationSpeedMultiplier)
        : Math.abs(puck.x - c) > 10 &&
          (puck.x > c
            ? (aiPaddle.x += aiPaddle.speed * simulationSpeedMultiplier)
            : (aiPaddle.x -= aiPaddle.speed * simulationSpeedMultiplier));
      aiPaddle.y += (50 - aiPaddle.y) * 0.05 * simulationSpeedMultiplier;
  ...
  aiPaddle.x = Math.min(
    Math.max(aiPaddle.x, aiPaddle.radius),
    canvas.width - aiPaddle.radius
  );
  aiPaddle.y = Math.min(
    Math.max(aiPaddle.y, aiPaddle.radius),
    canvas.height / 2 - aiPaddle.radius
  );
}
...
function collisionDetection() {
  if (
    puck.x + puck.dx > canvas.width - puck.radius ||
    puck.x + puck.dx < puck.radius
  )
    puck.dx = -puck.dx;
  if (
    (puck.y + puck.dy > canvas.height - puck.radius &&
      (puck.x <= (canvas.width - goal.width) / 2 ||
        puck.x >= (canvas.width + goal.width) / 2)) ||
    (puck.y + puck.dy < puck.radius &&
      (puck.x <= (canvas.width - goal.width) / 2 ||
        puck.x >= (canvas.width + goal.width) / 2))
  )
    puck.dy = -puck.dy;
  const dp = Math.hypot(puck.x - paddle.x, puck.y - paddle.y),
    da = Math.hypot(puck.x - aiPaddle.x, puck.y - aiPaddle.y);
  if (dp < puck.radius + paddle.radius) {
    const a = Math.atan2(puck.y - paddle.y, puck.x - paddle.x);
    puck.dx = Math.cos(a) * puck.speed;
    puck.dy = Math.sin(a) * puck.speed;
  }
  if (da < puck.radius + aiPaddle.radius) {
    const a = Math.atan2(puck.y - aiPaddle.y, puck.x - aiPaddle.x);
    puck.dx = Math.cos(a) * puck.speed;
    puck.dy = Math.sin(a) * puck.speed;
  }
  if (
...
function draw(currentTime) {
  ...
  drawTable();
  drawPaddle(paddle.x, paddle.y, paddle.radius);
  drawPaddle(aiPaddle.x, aiPaddle.y, aiPaddle.radius);
  drawPuck();
  drawExplosion();
  const t = Date.now(),
    s = (t - lastScoreTime) * simulationSpeedMultiplier;
  if (s <= 3000)
    s < 1000
      ? drawCountdown("3")
      : s < 2000
      ? drawCountdown("2")
      : s < 3000 && drawCountdown("1");
  ...
  if (goalEffect.active) {
    ...
    if (e < 1000) {
      ...
      ctx.textAlign = "center";
      ctx.fillText(
        goalEffect.scorer === "Player" ? "Player scored!" : "AI scored!",
        canvas.width / 2,
        canvas.height / 4
      );
